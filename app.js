...
(() => {
  ...
  function standardiseRows(rows) {
    ...
    for (let r = headerIndex + 1; r < rows.length; r++) {
      ...
      const isControl = /control/i.test(treatmentName) || /^t0+$/i.test(treatmentId) || /^no change$/i.test(String(row[idx['Practice Change Label']] || '').trim());
      cleaned.push({
        treatmentId,
        replicateId,
        treatmentName,
        practiceLabel: String(row[idx['Practice Change Label']] || '').trim(),
        isControl,
  ...
  function validate(rows) {
    ...
    if (!controlCount) messages.push('No control rows are marked TRUE.');
  ...
  function computeMetrics(avgYield, avgCost, sched, price, benefitAdj=0, costAdj=0) {
    ...
    return { pvBenefits, pvCosts, npv, bcr, gpm };
  }
  ...
  function renderRanking() {
    ...
    state.ranking.forEach((r) => {
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${r.rank}</td><td>${escapeHtml(r.treatmentName)}</td><td>${fmtNum(r.avgYield,2)}</td><td>${fmtMoney(r.avgCost)}</td><td>${fmtMoney(r.metrics.pvBenefits)}</td><td>${fmtMoney(r.metrics.pvCosts)}</td><td>${fmtMoney(r.metrics.npv)}</td><td>${fmtNum(r.metrics.bcr,2)}</td><td>${fmtPct(r.metrics.gpm)}</td><td>${fmtMoney(r.deltaVsControl)}</td>`;
      tbody.appendChild(tr);
    });
  }

  function buildPrompt() {
    const schedule = describeSchedule(discountSchedule());
    const assumptions = `Assumptions\nGrain price: ${fmtMoney(n($('grainPrice')?.value) || 0)} per tonne\nAnalysis years: ${String(n($('analysisYears')?.value) || 10)}\nDiscount schedule: ${schedule}`;
    const rankingRows = state.ranking.slice(0, 8).map((r) => `| ${r.rank} | ${r.treatmentName} | ${fmtNum(r.avgYield,2)} | ${fmtMoney(r.avgCost)} | ${fmtMoney(r.metrics.pvBenefits)} | ${fmtMoney(r.metrics.pvCosts)} | ${fmtMoney(r.metrics.npv)} | ${fmtNum(r.metrics.bcr,2)} | ${fmtPct(r.metrics.gpm)} | ${fmtMoney(r.deltaVsControl)} |`).join('\n');
    ...
    return `Use the results below to write a professional and human sounding output for farmers, advisers and project stakeholders. Do not use em dashes. Avoid bullet points in the main narrative unless absolutely necessary. Write clearly and naturally, not in an artificial or generic AI style. Use Australian spelling.\n\nPrepare the following in one response.\nFirst, write a concise interpretation of the main findings in full paragraphs.\nSecond, provide a clean results table.\nThird, write a short farmer-friendly report.\nFourth, write a one-page policy brief style summary.\nFifth, explain what the sensitivity analysis implies in plain language.\n\n${assumptions}\n\nTreatment ranking\n| Rank | Treatment | Average yield (t/ha) | Average direct costs ($/ha) | PV benefits ($/ha) | PV costs ($/ha) | NPV ($/ha) | BCR | Gross profit margin (%) | Difference versus control ($/ha) |\n|---|---|---:|---:|---:|---:|---:|---:|---:|---:|\n${rankingRows || 'No ranking available yet.'}\n\nSensitivity analysis\n| Scenario | Discount rate (%) | Grain price ($/t) | Benefits change (%) | Costs change (%) | NPV ($/ha) | BCR | Gross profit margin (%) |\n|---|---:|---:|---:|---:|---:|---:|---:|\n${sensRows || 'No sensitivity results available yet.'}`;
  }
  ...
  function openReport() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>SOIL CRC BCA Report</title><style>
      body{font-family:Arial,Helvetica,sans-serif;color:#1f2b1f;margin:0;background:#f6f8f5} .wrap{max-width:1120px;margin:0 auto;padding:28px}
      .site-header{background:#ffffff;border:1px solid #d9e2d4;border-radius:18px;padding:18px 22px;margin-bottom:20px}
      .brand-wrap{display:flex;align-items:center;justify-content:space-between;gap:18px} .brand-copy .eyebrow{font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#2b6c56;font-weight:700}
      .brand-copy h1{margin:2px 0 0;font-size:28px;line-height:1.15} .brand-logo-uon{height:56px} .brand-logo-soil{height:34px}
      .box{background:#fff;border:1px solid #d9e2d4;border-radius:16px;padding:20px;margin-bottom:18px}
      .cover{background:#0f3b2e;color:#fff} .cover h2{margin:0 0 10px;font-size:24px} .cover p{margin:6px 0;line-height:1.55}
      ...
      .two{display:grid;grid-template-columns:1fr 1fr;gap:18px} @media print{body{background:#fff} .box{break-inside:avoid}} @media (max-width:850px){.two{grid-template-columns:1fr} .brand-wrap{flex-direction:column;align-items:flex-start}}
    </style></head><body><div class="wrap">
      <header class="site-header"><div class="brand-wrap"><img src="${uonSrc}" alt="University of Newcastle logo" class="brand-logo-uon"><div class="brand-copy"><div class="eyebrow">SOIL CRC</div><h1>Benefit-Cost Analysis Tool</h1></div><img src="${soilSrc}" alt="SOIL CRC logo" class="brand-logo-soil"></div></header>
      <div class="box cover"><h2>SOIL CRC BCA Analytical Tool</h2><p>Version 2026.01</p><p>Publisher and Developer: Frank Wogbe Agbola, The University of Newcastle, Australia</p><p>Co-Developer: Dr Mesfin Genie, The University of Newcastle, Australia</p><p>Email any feedback, comments and suggestions, quoting version 2026.01, to Frank.Agbola@newcastle.edu.au</p></div>
      <div class="two"><div class="box"><h2>Selected settings</h2>${assumptions}</div><div class="box"><h2>Headline interpretation</h2><p>${escapeHtml($('headlineSummary')?.textContent || '')}</p></div></div>
      <div class="box"><h2>Treatment ranking</h2><table><thead><tr><th>Rank</th><th>Treatment</th><th>Average yield (t/ha)</th><th>Average direct costs ($/ha)</th><th>PV benefits ($/ha)</th><th>PV costs ($/ha)</th><th>NPV ($/ha)</th><th>BCR</th><th>Gross profit margin (%)</th><th>Difference versus control ($/ha)</th></tr></thead><tbody>${rankingRows}</tbody></table></div>
      <div class="box"><h2>Sensitivity analysis</h2><table><thead><tr><th>Scenario</th><th>Discount rate (%)</th><th>Grain price ($/t)</th><th>Benefits change (%)</th><th>Costs change (%)</th><th>PV benefits ($/ha)</th><th>PV costs ($/ha)</th><th>NPV ($/ha)</th><th>BCR</th><th>Gross profit margin (%)</th></tr></thead><tbody>${sensitivityRows}</tbody></table></div>
    ...
    if (!w) {
      toast('Popup blocked', 'Allow popups for this site to open the printable report.', 'warn');
      return;
    }
    w.document.open();
    w.document.write(html);
    w.document.close();
    toast('Report opened', 'The printable report has been opened in a new window.', 'info');
  }
  ...
  function bind() {
    ...
    $('runSensitivityBtn')?.addEventListener('click', () => renderSensitivity(true));
